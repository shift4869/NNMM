...
from NNMM.process.process_download import ProcessDownload, ProcessDownloadThreadDone

logger = getLogger("NNMM.process.process_download")
...
class TestProcessDownload(unittest.TestCase):
    ...
    def test_PDL_download_thread(self):
        """ProcessDownloadの動画ダウンロードワーカーを実行する処理をテストする
        """
        with ExitStack() as stack:
            mockli = stack.enter_context(patch.object(logger, "info"))
            mockle = stack.enter_context(patch.object(logger, "error"))
            mockworker = stack.enter_context(patch("NNMM.process.process_download.ProcessDownload.download_thread_worker"))
    ...
    def test_PDL_download_thread_worker(self):
    ...
    def test_PDLTDrun(self):
        ...
        with ExitStack() as stack:
            ...
            pdltd = ProcessDownloadThreadDone()
