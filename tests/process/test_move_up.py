...
class TestMoveUp(unittest.TestCase):
    def setUp(self):
        self.enterContext(patch("nnmm.process.move_up.logger.info"))
        self.enterContext(patch("nnmm.process.move_up.logger.error"))
    ...
    def test_callback(self) -> Result:
        ...
        def pre_run(params: Params) -> MoveUp:
            instance = self._get_instance()
        ...
        def post_run(actual: Result, instance: MoveUp, params: Params) -> None:
            ...
            if params.selected_mylist_row_index <= 0:
                # 最上のマイリストのため上に動かせない
                instance.mylist_db.assert_not_called()
                instance.window.list_widget.assert_not_called()
                return
        ...
        params_list = [
            Params("valid", "valid", 1, Result.success),
            Params("valid", "invalid", 0, Result.failed),
            Params("valid", "valid", 0, Result.failed),
            Params("invalid", "invalid", 1, Result.failed),
        ]
