...
class TestProcessInfo(unittest.TestCase):
    def test_init(self):
        ...
        actual = ProcessInfo(process_name, window, mylist_db, mylist_info_db)
        ...
        params_list = [
            (-1, window, mylist_db, mylist_info_db),
            (process_name, -1, mylist_db, mylist_info_db),
            (process_name, window, -1, mylist_db, mylist_info_db),
            (process_name, window, -1, mylist_info_db),
            (process_name, window, mylist_db, -1),
        ]
        for params in params_list:
            with self.assertRaises(ValueError):
                actual = ProcessInfo(params[0], params[1], params[2], params[3])
    ...
    def test_repr(self):
        ...
        mylist_db = MagicMock(spec=MylistDBController)
        mylist_info_db = MagicMock(spec=MylistInfoDBController)
        process_info = ProcessInfo(process_name, window, mylist_db, mylist_info_db)
        ...
        expect = f"ProcessInfo({name_str}, {window_str}, {mylist_db_str}, {mylist_info_db_str})"
    ...
    def test_create(self):
        ...
        def return_main_window(mylist_db_flag=True, mylist_info_db_flag=True):
        ...
        self.assertIsInstance(actual, ProcessInfo)
        ...
        for params in params_list:
            main_window = return_main_window(params[0], params[1])
            with self.assertRaises(ValueError):
                actual = ProcessInfo.create(process_name, main_window)
