...
from nnmm.process.config import ConfigBase, ConfigBrowserPath
from nnmm.process.value_objects.process_info import ProcessInfo
from nnmm.util import Result


class TestConfigBrowserPath(unittest.TestCase):
    def setUp(self):
    ...
    def test_init(self):
        process_config_base = ConfigBrowserPath(self.process_info)

        self.assertEqual(self.process_info, process_config_base.process_info)
        self.assertEqual(ConfigBrowserPath.CONFIG_FILE_PATH, ConfigBase.CONFIG_FILE_PATH)
        self.assertEqual(None, ConfigBase.config)

    def test_create_component(self):
        """ConfigBrowserPath.create_component が QPushButton("参照") を作成しクリック接続すること"""
        ...
        instance = ConfigBrowserPath(self.process_info)
    ...
    def test_callback(self):
        ...
        def pre_run(params: Params) -> ConfigBrowserPath:
            ...
            if params.is_use_tbox:
                ...
                self.process_info.window.tbox_browser_path = tbox_mock
            else:
                self.process_info.window = MagicMock(spec=QDialog)  # tbox_browser_path を持たない

            dialog_inst.getOpenFileName.reset_mock()
            if params.kind_browser_path_str == "valid_existing_path":
                dialog_inst.getOpenFileName.return_value = (self.tmp, "filter_str")
            elif params.kind_browser_path_str == "valid_non_existing_path":
                dialog_inst.getOpenFileName.return_value = (self.tmp.name + "_nonexistent", "filter_str")
            elif params.kind_browser_path_str == "empty_path":
                dialog_inst.getOpenFileName.return_value = ("", "filter_str")
            else:  # params.kind_browser_path_str == "invalid_path":
                dialog_inst.getOpenFileName.return_value = (-1, "filter_str")

            return ConfigBrowserPath(self.process_info)

        def post_run(actual, instance, params: Params):
            self.assertEqual(params.result, actual)
            if params.is_use_tbox:
                dialog_inst.getOpenFileName.assert_called_once()
                if params.kind_browser_path_str == "valid_existing_path":
                    self.process_info.window.tbox_browser_path.setText.assert_called_once()
                else:
                    self.process_info.window.tbox_browser_path.setText.assert_not_called()
            else:
                dialog_inst.getOpenFileName.assert_not_called()
                self.assertFalse(hasattr(self.process_info.window, "tbox_browser_path"))
