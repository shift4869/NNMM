...
from nnmm.process.config import ConfigBase, ConfigDBSavePath
from nnmm.process.value_objects.process_info import ProcessInfo
from nnmm.util import Result


class TestConfigDBSavePath(unittest.TestCase):
    def setUp(self):
    ...
    def test_init(self):
        process_config_base = ConfigDBSavePath(self.process_info)

        self.assertEqual(self.process_info, process_config_base.process_info)
        self.assertEqual(ConfigDBSavePath.CONFIG_FILE_PATH, ConfigBase.CONFIG_FILE_PATH)
        self.assertEqual(None, ConfigBase.config)

    def test_create_component(self):
        """ConfigDBSavePath.create_component が QPushButton("参照") を作成しクリック接続すること"""
        ...
        instance = ConfigDBSavePath(self.process_info)
    ...
    def test_callback(self):
        ...
        def pre_run(params: Params) -> ConfigDBSavePath:
            ...
            if params.is_use_tbox:
                ...
                self.process_info.window.tbox_db_path = tbox_mock
            else:
                self.process_info.window = MagicMock(spec=QDialog)  # tbox_db_path を持たない
            ...
            return ConfigDBSavePath(self.process_info)
        ...
        def post_run(actual, instance, params: Params):
            ...
            if params.is_use_tbox:
                ...
                if params.kind_browser_path_str == "valid_existing_path":
                    self.process_info.window.tbox_db_path.setText.assert_called_once()
                else:
                    self.process_info.window.tbox_db_path.setText.assert_not_called()
            else:
                dialog_inst.getOpenFileName.assert_not_called()
                self.assertFalse(hasattr(self.process_info.window, "tbox_db_path"))
