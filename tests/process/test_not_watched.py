...
class TestNotWatched(unittest.TestCase):
    def setUp(self):
        self.enterContext(patch("nnmm.process.not_watched.logger.info"))
        self.enterContext(patch("nnmm.process.not_watched.logger.error"))
    ...
    def _get_instance(self) -> NotWatched:
    ...
    def test_callback(self) -> Result:
        ...
        def pre_run(params: Params) -> NotWatched:
            instance = self._get_instance()
        ...
        def post_run(actual: Result, instance: NotWatched, params: Params) -> None:
            ...
            self.assertEqual(
                [
                    call.update_status(video_id, mylist_url, "未視聴"),
                ],
                instance.mylist_info_db.mock_calls,
            )

            self.assertEqual(
                [call.update_include_flag(mylist_url, True)],
        ...
        params_list = [
            Params("valid", 0, Result.success),
            Params("valid", -1, Result.success),
            Params("invalid", 0, Result.failed),
        ]
