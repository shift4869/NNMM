...
class TestVideoPlay(unittest.TestCase):
    def setUp(self):
        self.enterContext(patch("nnmm.process.video_play.logger.info"))
        self.enterContext(patch("nnmm.process.video_play.logger.error"))
    ...
    def _get_instance(self) -> VideoPlay:
    ...
    def test_callback(self) -> Result:
        ...
        mock_subprocess = self.enterContext(patch("nnmm.process.video_play.subprocess.run"))
        mock_popup = self.enterContext(patch("nnmm.process.video_play.popup"))
        mock_watched = self.enterContext(patch("nnmm.process.video_play.Watched"))
        self.enterContext(patch("nnmm.process.video_play.Path"))
        Params = namedtuple(
            "Params",
            [
        ...
        def pre_run(params: Params) -> VideoPlay:
            instance = self._get_instance()
        ...
        def post_run(actual: Result, instance: VideoPlay, params: Params) -> None:
