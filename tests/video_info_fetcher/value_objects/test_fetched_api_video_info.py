...
class TestFetchedAPIVideoInfo(unittest.TestCase):
    def test_FetchedAPIVideoInfoInit(self):
        ...
        with self.assertRaises(FrozenInstanceError):
            fvi_api = FetchedAPIVideoInfo(
                no, video_id_list, title_list, uploaded_at_list, video_url_list, username_list
            )
    ...
    def test_is_valid(self):
        ...
        with self.assertRaises(TypeError):
            fvi_api = FetchedAPIVideoInfo(no, None, title_list, uploaded_at_list, video_url_list, username_list)
        # TitleList 指定が不正
        with self.assertRaises(TypeError):
            fvi_api = FetchedAPIVideoInfo(no, video_id_list, None, uploaded_at_list, video_url_list, username_list)
        # UploadedAtList 指定が不正
        with self.assertRaises(TypeError):
            fvi_api = FetchedAPIVideoInfo(no, video_id_list, title_list, None, video_url_list, username_list)
        # VideoURLList 指定が不正
        with self.assertRaises(TypeError):
            fvi_api = FetchedAPIVideoInfo(no, video_id_list, title_list, uploaded_at_list, None, username_list)
        # UsernameList 指定が不正
        with self.assertRaises(TypeError):
            fvi_api = FetchedAPIVideoInfo(no, video_id_list, title_list, uploaded_at_list, video_url_list, None)
        # UsernameList 指定が不正
        with self.assertRaises(TypeError):
            fvi_api = FetchedAPIVideoInfo(no, video_id_list, title_list, uploaded_at_list, video_url_list, None)
        # no 指定が不正
        with self.assertRaises(ValueError):
            fvi_api = FetchedAPIVideoInfo(
                [], video_id_list, title_list, uploaded_at_list, video_url_list, username_list
            )
        # list の長さが同じでない
