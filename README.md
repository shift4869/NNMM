...
1. 登録したマイリストの動画一覧は以下のいずれかで更新される（RSS, APIを用いて1回で最大約30件ずつ蓄積）
    - 動画一覧が右ペインに表示されているときに左上の更新ボタンを押す
    - インターバル更新ボタン押下時
        - 前回の更新から「更新確認インターバル」以上期間が開いていた場合に更新対象となる
        - それぞれのマイリストの「更新確認インターバル」は右クリック→「情報表示」から確認・設定できる
    - すべて更新ボタン押下時
    - オートリロード時
        - 設定タブより間隔を指定できる
